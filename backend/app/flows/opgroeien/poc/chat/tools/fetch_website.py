...
fetch_website = StructuredTool.from_function(
    ...
    name="fetch_website",
    description="Fetches the current content of a webpage and converts it to Markdown. The tool automatically removes navigation elements, footers, scripts, and other non-content elements. ⚠️ UITZONDERLIJK GEBRUIK: Gebruik deze tool ALLEEN in de volgende uitzonderlijke gevallen: 1. De gebruiker vraagt EXPLICIET naar informatie op een specifieke website (opgroeien.be of vlaanderen.be), 2. Interne vector search heeft GEEN resultaten opgeleverd en de vraag kan niet worden beantwoord met interne kennis, 3. Er is een specifieke, uitzonderlijke behoefte die niet kan worden ingevuld door procedures_vector_search of regelgeving_vector_search. Gebruik deze tool NIET voor routine validatie, verificatie, of om te controleren of interne informatie actueel is. Interne kennis is de primaire en standaard bron. Gebruik ALTIJD eerst procedures_vector_search en regelgeving_vector_search. Provide the full URL (including https://). The output is a clean Markdown version of the main content of the page.",
)
